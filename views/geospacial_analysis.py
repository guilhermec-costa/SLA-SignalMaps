...
from polygons import polygons, tmp_coordinates
import pandas as pd
...
def geo_analysis(results: querie_builder.Queries, main_data=None):
    ...
    if 'general_data' not in st.session_state:
        st.session_state.general_data = pd.DataFrame()
    ...
    with st.form(key='submit_sla_form'):
        ...
        filtro_BU = c_BU.multiselect('Business Unit', options=st.session_state.general_data['Unidade de Negócio - Nome'].unique(), default=st.session_state.general_data['Unidade de Negócio - Nome'].unique())
        st.session_state.city_filter = c_city.multiselect('City', options=st.session_state.general_data['Cidade - Nome'].unique(), default=st.session_state.general_data['Cidade - Nome'].unique())
        ...
        if submit_form:
            ...
            #agrupado_por_condo.IEF = agrupado_por_condo.IEF.apply(lambda x: round(x, 2))
            filtered_group.df = filtered_data.df.groupby(by=['Unidade de Negócio - Nome','Cidade - Nome', 'Grupo - Nome', 'Endereço']).agg({'IEF':np.mean, 'Matrícula':'count', 'Latitude':np.mean, 'Longitude':np.mean, 'data snapshot':np.max}).reset_index()
    ...
    # 
    # filtered_data = Filters(st.session_state.main_data)
    # filtered_group = Filters(st.session_state.grouped_data)
    ...
    with st.expander('Edit gateway options'):
        with st.form('gtw_form', clear_on_submit=False):
            ...
            personalized_gtw = st.session_state.general_data[(st.session_state.general_data['Endereço'].isin(st.session_state.extra_selected_address))|(st.session_state.grouped_data['Grupo - Nome'].isin(st.session_state.extra_selected_residence))]
    ...
    if st.session_state.gtw_filters:
        ...
        with st.spinner('Calculating polygons...'):
            with ThreadPoolExecutor(4) as executor:
                for n in stqdm(range(len(lat_list))):
                    ...
                    args = [(index, row[-1], current_polygon) for index, *row in st.session_state.main_data.itertuples()]
                    results = executor.map(polygons.check_if_pol_contains, args)
                    contained_index.extend([i for i in results if i is not None])
                    st.session_state.main_data = st.session_state.main_data[~st.session_state.main_data.index.isin(contained_index)]
                    ...
                    if st.session_state.polygon_df is not None:
                        sla_maps.add_traces_on_map(st.session_state.grouped_points_figure, another_data=st.session_state.polygon_df, fillcolor='rgba(20, 2222, 169, 0.4)', name=df_filtered_per_points.loc[n:n, 'Endereço'].values[0])
                        sla_maps.add_traces_on_map(st.session_state.grouped_sla_figure, another_data=st.session_state.polygon_df, fillcolor='rgba(20, 222, 169, 0.4)', name=df_filtered_per_points.loc[n:n, 'Endereço'].values[0])                 
                        sla_maps.add_traces_on_map(st.session_state.all_points_figure, another_data=st.session_state.polygon_df, fillcolor='rgba(20, 222, 169, 0.4)', name=df_filtered_per_points.loc[n:n, 'Endereço'].values[0])
        
        affected_points = st.session_state.general_data.loc[contained_index]
        ...
        if df_filtered_per_points.shape[0] >= 1:
            ...
            sla_prevision.metric('Improvement preview over the general SLA %', value=f'{round(qty_that_is_contained / st.session_state.main_data.shape[0] * 100, 2)}%')

            with st.expander('Addresses and installations affected'):
                st.write(affected_points)
                st.write(st.session_state.grouped_data)

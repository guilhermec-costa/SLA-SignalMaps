...
def metrics_boxplot(data):
    fig = go.Figure()
    fig.add_trace(go.Box(x=data['snapshot_date'], y=data['sla_mean'], boxmean='sd', marker=dict(color='#4FFF64')))
    ...
    fig.update_xaxes(tickfont=dict(size=16, family='roboto'), rangeslider_visible=True, range=[np.min(data['snapshot_date'] + datetime.timedelta(days=20)),

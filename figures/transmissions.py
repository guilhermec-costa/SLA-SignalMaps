...
def daily_transmissions(data):
    fig = make_subplots(rows=1, cols=2, vertical_spacing=0.3, print_grid=True,
    ...
    for idx, bu in enumerate(data['name'].unique()):
        ...
        fig.add_trace(go.Bar(x=filtered_data["snapshot_date"], y=filtered_data['qtd_transmissoes'], legendgroup=f'bu_{bu}', 
                                 ...
                                 textposition='outside', width=0.4, textfont=dict(size=14, family='roboto')),
        ...
        fig.add_trace(go.Bar(x=filtered_data["snapshot_date"], y=filtered_data['pontos_ativos'], name='Active points', showlegend=showlegend,
                             marker=dict(color='#51DB91'), text=filtered_data['pontos_ativos'], textposition='outside', width=0.4,
    ...
    fig.update_layout(font=dict(family='roboto'), height=600, barmode='group', bargap=0.099, template='presentation',
                    legend=dict(

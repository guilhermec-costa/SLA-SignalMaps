...
def individual_com_figure(data:pd.DataFrame):
    fig = go.Figure()
    ...
    fig.update_layout(barmode='group', height=650, uniformtext_minsize=18, font=dict(family='roboto'), template='presentation',
                      title=dict(text='SLA % comparison between two dates', font=dict(family='roboto', size=25), x=0.5, xanchor='center', y=0.95, yanchor='top'))

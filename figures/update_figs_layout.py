...
def alter_legend(fig, title):
    fig.update_layout(legend=dict(font=dict(size=13, color="black", family='roboto'), bgcolor="#D5D5D1",
                                  bordercolor="#70704C", borderwidth=2,
                                  title=dict(text=f'{title}', font=dict(size=16, color='black', family='roboto')), font_family='roboto'))
...
def prettify_fig(fig):
    fig.update_xaxes(title=dict(text=None, font=dict(family='roboto', size=18)), tickfont=dict(family='roboto', size=14))
